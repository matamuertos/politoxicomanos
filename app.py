...
class User(db.Model):
    ...
    # Flag to control whether a user is allowed to post comments.  If True, the
    # user will be prevented from submitting new comments.  This can be toggled
    # by administrators via the admin tools.  Defaults to False for normal
    # operation.
...
class Contribution(db.Model):
    ...
    approved = db.Column(db.Boolean, default=None)  # None = pending, True = approved, False = rejected
...
# New model to handle votes on contributions
...
def add_notification(user_id: int, message: str):
    """Create and commit a new notification for a user."""
...
def send_email(to_email: str, subject: str, body: str) -> None:
    """
    Send an email using SMTP settings defined in environment variables.
    Environment variables:
        ...
        MAIL_USE_TLS: 'True' or 'False' for STARTTLS (default: False)
    If sending fails, an error is printed. During development, the confirmation link
    will also be printed to the console via flash messages.
    """
...
def inject_user():
    ...
    if user_id:
        user = User.query.get(user_id)
        # If the user ID is present in the session but no longer exists in the database,
        # treat the session as logged out to prevent AttributeError on None.
        if user:
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not username or not email or not password:
            ...
            # Generate confirmation token and set unconfirmed
            ...
            try:
                ...
                # If email fails, show link for manual confirmation (útil en desarrollo)
...
def view_contribution(contrib_id):
    ...
    if request.method == 'POST':
        ...
        # do not allow muted users to post comments
...
def vote_comment(comment_id, value):
    ...
    comment.votes += delta
    # update comment author points (only positive votes count) -> delta positive increments points, negative decrements
...
#
# The following routes allow site administrators to manage community
# interactions.  These endpoints are protected by the ``admin_required``
# decorator so only users with ``is_admin=True`` may access them.

@app.route('/admin/delete_comment/<int:comment_id>', methods=['POST'])
@admin_required
def delete_comment(comment_id):
    """
    Permanently remove a comment from the database.  Admins can use this
    endpoint to clean up spam or otherwise inappropriate content.  After
    deletion, the user is redirected back to the parent contribution.
    """
    comment = Comment.query.get_or_404(comment_id)
...
def mute_user(user_id):
    """
    Toggle the muted status of a user.  When muted, a user may not submit
    new comments.  Muting does not affect the ability to submit new
    contributions.  Admins cannot mute/unmute themselves.
    """
    target_user = User.query.get_or_404(user_id)
    current_admin_id = session.get('user_id')
    # prevent administrators from muting/unmuting themselves
...
def approve_contribution(contrib_id):
    ...
    if contrib.approved is not None:
        ...
        contrib.author.points += 10  # award points
        db.session.commit()
...
@app.route('/trending')
def trending():
    contributions = Contribution.query.filter_by(approved=True).order_by(Contribution.votes.desc(), Contribution.created_at.desc()).all()
    return render_template('trending.html', contributions=contributions)
...
def feed():
    ...
    if followed_ids:
        contributions = Contribution.query.filter(Contribution.user_id.in_(followed_ids), Contribution.approved == True).order_by(Contribution.created_at.desc()).all()
    else:
        contributions = []
    return render_template('feed.html', contributions=contributions)
...
def categories_list():
    """
    Show a list of all categories with a link to each category page. Useful to help
    visitors discover content by category. Categories can be extended later via
    the database.
    """
...
# Show contributions of a category
...
def vote_contribution(contrib_id, value):
    if value not in (1, -1):
        abort(400)
    contrib = Contribution.query.get_or_404(contrib_id)
    # Only approved contributions can be voted
    if contrib.approved is not True:
        flash('Solo puedes votar aportes aprobados.', 'danger')
        return redirect(url_for('view_contribution', contrib_id=contrib.id))
    user_id = session['user_id']
    # Authors cannot vote their own contributions
...
def embed_video(link: str) -> str:
    """
    Convert a video link into an embeddable URL. Supports YouTube.
    If the link is from YouTube (either youtu.be or youtube.com), extract the video ID
    and return the embed URL. Otherwise, return the original link.
    """
    if not link:
        return ''
    try:
        ...
        if 'youtube.com' in link:
            # typical format: https://www.youtube.com/watch?v=VIDEO_ID
            ...
            if video_id:
                return f'https://www.youtube.com/embed/{video_id}'
        # return original link for unsupported sites (it may still embed)
        return link
    except Exception:
        return link
